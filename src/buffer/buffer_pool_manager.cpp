 ...
 std::scoped_lock<std::mutex> lock(latch_);
  for (size_t i = 0; i < pool_size_; i++) {
    ...
    if (page_tmp->GetPageId() == page_id) {
      auto promise1 = disk_scheduler_->CreatePromise();
      auto future1 = promise1.get_future();
      disk_scheduler_->Schedule({true,page_tmp->GetData(),page_tmp->GetPageId(),std::move(promise1)});
      future1.get();
      page_tmp->is_dirty_ = false;
      return true;
    }
...
void BufferPoolManager::FlushAllPages() {
  ...
  for (size_t i = 0; i < pool_size_; i++) {
    Page *page_tmp = &pages_[i];
    if (page_tmp->GetPageId() != INVALID_PAGE_ID) {
      auto promise1 = disk_scheduler_->CreatePromise();
      auto future1 = promise1.get_future();
      disk_scheduler_->Schedule({true,page_tmp->GetData(),page_tmp->GetPageId(),std::move(promise1)});
      future1.get();
